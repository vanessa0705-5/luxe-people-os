...
export async function exportarExcel(registro: RateioFolhaRegistro): Promise<void> {
  if ((registro.resultado.resumo as any).tipo === "encargos_documentos") {
    const { exportarExcelEncargos } = await import("@/lib/encargos-documentos");
    return exportarExcelEncargos(registro);
  }
